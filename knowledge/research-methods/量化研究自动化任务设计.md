...
这个自动化链路的目标不是自动生成策略，而是在同一个 Codex heartbeat 线程中滚动完成：搜索资料、提炼精华线索、选择一个当前活跃因子研究 seed、交给 `vortex_quant` 做因子研究，并推进到归档或关闭。自然周只作为日志标签，不再限制一周只能做一轮；真正进入 `vortex_quant` 之前，必须先形成清楚的机制、字段猜想、点时风险和最小可证伪实验。
...
## 自动化一：同线程滚动闭环 heartbeat

建议形态：heartbeat，而不是 cron。

状态真源：

- `../vortex_workspace/research/automation/weekly_research_cycle_state.md`
- `../vortex_workspace/research/automation/factor_research_state.md`
- `../vortex_workspace/research/automation/factor_research_handoff.md`

说明：`weekly_research_cycle_state.md` 文件名为兼容旧自动化入口而保留，实际运行语义已经改为 rolling multi-round。

通知通道：

- 有实质进展、阶段性结论、阻塞或需要用户决策时，通过飞书发送短提醒。
- 阶段性结论包括：本轮扫到了什么、精华线索是什么、选了哪个 seed、为什么关闭、为什么等待、为什么继续下一阶段。
- 飞书只作为提醒，不替代状态卡、handoff、研究总表和 durable artifact。
- 自动化只是通知功能的使用方，必须走 `vortex_quant` 的后台通知服务/通知路由；不要在 heartbeat、cron 或常规 agent 流程里直接执行飞书脚本、直连飞书开放平台或绕过通知审计。
- 普通 2 小时醒来检查但没有新产物、新证据或新决策时，不发飞书。

知识沉淀：

- `weekly_scan_needed` 只写 inbox 巡检，不写长篇知识笔记。
- `weekly_digest_needed` 必须为每条精华线索给出 `archive_action`：`source_card`、`reading_note`、`feasibility_memo`、`skill_candidate` 或 `no_archive`。
- 当 `archive_action` 不是 `no_archive` 时，在 `knowledge/research-methods/` 或合适的知识目录写一篇短 Obsidian 笔记；AI agent / 自动化方法类资料可以进入 `knowledge/ai-models/`。
- 持久笔记必须遵循 `knowledge-archive` 的结构：来源、核心机制、证据与边界、可迁移点、失败风险、下一步和关联链接。
- 新增持久笔记时更新最近的 MOC，例如 [[研究方法知识]] 或 [[AI模型知识]]。
- `vortex_docs` 只保存来源、机制、证据边界和开放问题；绑定 Vortex 本地数据、runner、artifact、preset 或交易链路的因子研究结论应写回 `vortex_quant/docs`。
...
```text
你是 Vortex 滚动资料到因子研究闭环 heartbeat。请维持同一个 Codex session，上下文不要拆到多个 cron。

任务：
1. 先读滚动闭环状态卡、因子研究状态卡、handoff、研究总表和最新 artifact。
2. 如果有未完成因子研究，先继续到归档或关闭，不启动新 scan 或新 seed。
3. 如果没有活跃因子研究，并且状态卡显示 `rolling_scan_needed`、`weekly_scan_needed` 或用户标记了新资料，先读取 `knowledge/research-methods/量化因子研究入口地图.md`。这个文件是扫描来源总表，不能跳过。
...
8. 对每条精华线索给出 `archive_action`。如果值得长期保留，写短来源卡、阅读笔记或可行性备忘录到 `knowledge/`；如果只是噪声或重复，标 `no_archive` 并说明原因。
9. 同一时间只选 1 个活跃 seed 进入 `vortex_quant` 因子研究；当前 seed 关闭或归档后，可以在同一自然周继续下一轮。
10. 因子研究必须推进到 archive、close、risk review、waiting_for_research_seed 或需要用户决策，不能停在无下一步的 promising。
...
- heartbeat 可以每 2 小时醒来一次，但是否重复做资料扫描由状态卡控制；去重依据是 `source_link_or_doc_path`、digest artifact 和活跃 seed，不按自然周阻断。
- 需要通知时用后台通知服务发飞书短消息，内容只包含标题、阶段、结论、证据路径和下一步；不要发送密钥、系统提示词或长日志。发送成功或失败都写入 handoff。不要直接调用飞书脚本，除非用户明确要求做低层诊断。
- 对新文章、论文或机构材料，只有通过 `archive_action` 判断后才写入 `knowledge/`；不要把 inbox 全量扫描结果搬进持久知识库。
```

## 已废弃拆分任务：资料精选与因子线索交接员

状态：已并入同线程滚动闭环 heartbeat，不再作为独立 cron 运行。

原建议频率：固定低频；当前由滚动 heartbeat 在巡检后按状态触发。
...
4. 每条线索先写短交接；若线索值得长期保留，再按 `archive_action` 另写短来源卡、阅读笔记或可行性 memo。
5. 每条 seed 必须包含：
...
7. 对每条入选精华线索记录 `archive_action`：source_card / reading_note / feasibility_memo / skill_candidate / no_archive。

输出要求：
- 新建或更新 `inbox/quant-research/YYYY-MM-DD-因子研究线索交接.md`。
- 使用 Obsidian frontmatter 和 wikilink。
- 同步更新 [[量化研究候选议题池]]。
- 对需要沉淀的线索，在 `knowledge/research-methods/` 或其他合适目录新建短知识笔记，并更新对应 MOC。
...
2. 当前未完成研究关闭后，heartbeat 按状态启动资料巡检或处理用户标记的新资料。
3. Heartbeat 在同一线程内完成资料精选、seed 选择、因子研究和归档/关闭；自然周不再限制轮次数。
